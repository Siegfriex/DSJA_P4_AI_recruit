...
def assign_repost_groups(
    ...
    active: list[dict] = []
    assignment: dict[str, tuple[str, str, bool, int]] = {}
    for row in result.to_dict(orient="records"):
        posting_id = str(row["postingId"])
        if not row["postingEligibleFlag"] or pd.isna(row["canonicalPostedAt"]):
            assignment[posting_id] = (f"UNRESOLVED_{posting_id}", posting_id, True, 0)
            continue
        match = None
    ...
    result["repostSequence"] = (
        result["postingId"].astype(str).map(lambda value: assignment[value][3])
    )
    result["dedupMode"] = (
