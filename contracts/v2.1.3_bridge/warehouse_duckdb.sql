-- COMPATIBILITY BRIDGE CANDIDATE
-- contractVersion: 2.1.3
-- baseContractVersion: 2.1.2
-- generatedAt: 2026-08-09
-- generatorVersion: clean-consolidation-0.1.0
...
-- v2.1.3 semantic lineage extensions. DuckDB cross-schema foreign keys remain
-- forbidden; their logical relationships are validated by p4 contract validate.
CREATE TABLE IF NOT EXISTS core.postingSourceBlock (
  sourceBlockId VARCHAR PRIMARY KEY,
  postingId VARCHAR NOT NULL,
  rawPostingId VARCHAR NOT NULL,
  assetId VARCHAR,
  sourceMode VARCHAR NOT NULL CHECK (sourceMode IN ('HTML', 'ACTIVITY_TEXT', 'OCR', 'MERGED')),
  sourceText VARCHAR NOT NULL,
  startChar INTEGER NOT NULL CHECK (startChar >= 0),
  endChar INTEGER NOT NULL CHECK (endChar >= startChar),
  pageNo INTEGER,
  bboxJson JSON,
  rawSha256 VARCHAR NOT NULL CHECK (regexp_full_match(rawSha256, '[0-9a-f]{64}')),
  ...
  empiricalAnalysisAllowed BOOLEAN NOT NULL DEFAULT false,
  promotionAllowed BOOLEAN NOT NULL DEFAULT false
);

CREATE TABLE IF NOT EXISTS core.sectionSourceBlock (
  sectionId VARCHAR NOT NULL,
  sourceBlockId VARCHAR NOT NULL,
  relationType VARCHAR NOT NULL CHECK (relationType IN ('EXACT_SPAN', 'DERIVED_SPAN')),
  dataVersion VARCHAR NOT NULL,
  contractVersion VARCHAR NOT NULL,
  crawlReleaseId VARCHAR NOT NULL,
  dataProvenance VARCHAR NOT NULL,
  empiricalAnalysisAllowed BOOLEAN NOT NULL DEFAULT false,
  promotionAllowed BOOLEAN NOT NULL DEFAULT false,
  PRIMARY KEY (sectionId, sourceBlockId)
);

CREATE TABLE IF NOT EXISTS core.semanticChunk (
  chunkId VARCHAR PRIMARY KEY,
  trackId VARCHAR NOT NULL,
  sectionId VARCHAR NOT NULL,
  sourceBlockId VARCHAR NOT NULL,
  chunkType VARCHAR NOT NULL CHECK (chunkType IN (
    'DUTY', 'REQUIREMENT', 'KNOWLEDGE', 'SKILL',
    'COMPETENCY_EXPERIENCE', 'PROCESS', 'OTHER'
  )),
  chunkText VARCHAR NOT NULL,
  normalizedText VARCHAR NOT NULL,
  ncsMappableFlag BOOLEAN NOT NULL,
  chunkingVersion VARCHAR NOT NULL,
  dataVersion VARCHAR NOT NULL,
  contractVersion VARCHAR NOT NULL,
  crawlReleaseId VARCHAR NOT NULL,
  dataProvenance VARCHAR NOT NULL,
